  ...
  return (
    <div className="flex flex-col rounded-[0.3rem] border-2 h-full w-full bg-purple-200 overflow-hidden">
      ...
      <div className=" flex justify-start items-start h-full flex-col mx-2 my-2">
        <p className="text-[0.8rem] text-purple-400">{ethnic.sponser}</p>
        <p className="text-[1rem] text-purple-700 mt-1">{ethnic.brandName}</p>
        <p className="text-[1.1rem] text-purple-800 mt-1">{ethnic.name}</p>
        <div className="flex justify-between gap-3 mt-1 text-[1.1rem]  ">
          <p className="text-purple-950">{ethnic.discountPrize}</p>
          <p className="text-purple-500 text-[0.85rem] line-through ">{ethnic.prize}</p>
          <p className="text-green-500 text-[0.9rem]">{ethnic.offer}</p>
        </div>
        <div className=" mt-1  text-purple-900">
          <div className=" w-full h-auto col-span-2 space-x-5">
            {ethnic.sizes.map((curElem, index) => {
              return (
